...
| Node.js 22 系（LTS） | Claude Code が補助ツールを動かすときに使う。受講者が直接打つことはない | `node -v` が `v22.x.x` であること（npm は Node.js に同梱されるので別途の導入は不要） |
...
> **注意:** 研修では **22 系**を使用する。公式サイトのトップに出る最新版は 22 系ではないことがあるため、以下のページから選ぶ。
...
> ```
>
> **`v22` 以外が表示された場合:** 別のバージョンの Node.js が入っている。既存の Node.js を削除してから入れ直すこと。
...
if ((Get-Command node -CommandType Application -ErrorAction SilentlyContinue) -and ((node -v) -like 'v22.*')) { "OK: Node.js $(node -v)" } else { "NG: Node.js 22 系なし" }
...
> **結果の見かた**
> - 最初の4つは、それぞれバージョン（`git version 2.x.x` `Python 3.12.x` `v22.x.x` `10.x.x`）が表示されれば正常
...
> - `Python 3.12` 以外・`v22` 以外のバージョンが表示された → PowerShell では正しくても Git Bash では別のものが見つかっている。PATH の並び順を確認すること
