...
![Windows（画面＝リモコン）と WSL / Linux（本体）が「Connect to WSL」でつながる関係図](images/WSLconnection.png)
...
![VSCode の画面構成（5つのエリア）](images/Layout.png)
...
> **スクリーンショットについて:** 画面右側に表示されている「CHAT」パネルは **GitHub Copilot**（Claude Code とは別の AI 機能）です。本ガイドでは使わないので、表示されていてもいなくても問題ありません。また、画像の WSL バージョン表記（例: Ubuntu-24.04）が、皆さんの画面（Ubuntu-22.04）と異なる場合がありますが、操作は同じです。
...
![コマンドパレット（Ctrl+Shift+P）を開いた状態](images/Commandpallette.png)
...
> **補足:** 研修で開くフォルダの場所は、研修資料に含まれる**事前セットアップ手順**に記載されています。ここでは「フォルダを開く操作の場所」を覚えておけば十分です。

![フォルダを開き、左のエクスプローラーにファイルツリーが表示された状態](images/Explorer.png)
...
![Ctrl+@ でターミナル（パネル）が開いた状態。下部に WSL の bash プロンプトが見える](images/Terminal.png)
...
![claude 起動後の対話画面](images/ClaudeStart.png)

> **注意:** Claude Code はバージョンの更新により、起動画面の見た目（表示される項目やレイアウト）が変わる場合があります。表示が多少違っても、ターミナルで `claude` を実行して対話画面に切り替われば起動成功です。
