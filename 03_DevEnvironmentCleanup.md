...
実行中、`y` の入力を2回求められる（「開始」と「ダウンロードフォルダ削除」）。いずれも `y` で進める。処理ではブラウザ（Chrome・Edge）・メモ帳・Zoom が終了され、ブラウザの Cookie・閲覧履歴・ブックマーク・タブ/セッション、メモ帳の未保存タブ、Zoom のログイン情報（次回起動時に再ログインが必要になる）、ダウンロードフォルダの中身、**ピクチャのスクリーンショット**（`ピクチャ\Screenshots`）が削除され、最後にごみ箱が空になる。
