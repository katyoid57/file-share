...
| **Windows側** | ブラウザ（Chrome・Edge）の Cookie・閲覧履歴・ブックマーク・タブ/セッション削除 ／ ダウンロードフォルダの全削除 ／ ごみ箱を空にする |
...
実行すると、ブラウザ（Chrome・Edge）が終了され、Cookie・閲覧履歴・ブックマーク・タブ/セッション（最近閉じたタブ等）が削除される。続いてダウンロードフォルダの全削除確認を求められるので `y` で実行する。最後にごみ箱が空になる。
