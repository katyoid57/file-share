...
> **「致命的なエラー」が出た場合のトラブルシューティング**
>
> 多くは以下のいずれかが原因です。上から順に確認してください。
>
> 1. **仮想化（Virtualization）が BIOS/UEFI で無効になっている** — 最も多い原因  
>    タスクマネージャー → 「パフォーマンス」タブ → CPU の「仮想化:」が **有効** になっているか確認。  
>    **無効** の場合は PC を再起動して BIOS/UEFI 設定画面に入り、Intel VT-x（または AMD-V / SVM）を有効化する。
>
> 2. **必要な Windows 機能が無効**  
>    「Windows の機能の有効化または無効化」を開き、以下にチェックを入れて再起動：  
>    - 仮想マシン プラットフォーム（Virtual Machine Platform）  
>    - Linux 用 Windows サブシステム（Windows Subsystem for Linux）
>
> 3. **Windows のバージョンが古い**  
>    Windows 10 21H2 以降、または Windows 11 が必要。Windows Update を最新まで適用する。
>
> 4. **コマンドプロンプトを管理者として起動していない**  
>    スタートメニューで「cmd」を右クリック → **管理者として実行** から再度試す。
>
> 上記を確認しても解決しない場合は、表示されたエラーメッセージ全文（特にエラーコード `0x...`）を控えて担当者に連絡してください。
